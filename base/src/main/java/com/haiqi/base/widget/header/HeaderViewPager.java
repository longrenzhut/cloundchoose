...
public class HeaderViewPager extends LinearLayout {
    ...
    boolean isAddHeader = false;//是否添加头部
    boolean isVerScroll = false;//是否设置左右滑动
    ...
    public HeaderViewPager(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.headervp);
        topOffset = a.getDimensionPixelSize(a.getIndex(R.styleable.headervp_hvp_topOffset), topOffset);
        isAddHeader = a.getBoolean(R.styleable.headervp_hvp_isAdd, false);
        isVerScroll = a.getBoolean(R.styleable.headervp_hvp_isVerScroll,false);
        a.recycle();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mVpHeight = MeasureSpec.getSize(heightMeasureSpec);
        //让测量高度加上头部的高度
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getPointerCount() > 1) {
            ...
            mScrollable.setScrollEnabled(false);
            return true;
        }
...
//                    mScrollable.isBottom();
                    ...
                    //垂直滑动
                    if(isVerScroll) {
                        mScrollable.setScrollEnabled(true);
                    }
                    verticalScrollFlag = true;
                }
                /**
        ...
        if(isAddHeader && currentY - mDownY < 0 && shiftY > shiftX ) {
            if (mCurY == maxY)
                super.dispatchTouchEvent(ev);
        }
        else{
...
//        if(isAddHeader) {
//            boolean canscroll = mScrollable.canScroll(mHeadHeight,mVpHeight,y);
//            if(!canscroll)
//                return;
//        }
        super.scrollTo(x, y);
