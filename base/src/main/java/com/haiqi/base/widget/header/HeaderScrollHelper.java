package com.haiqi.base.widget.header;
...
import com.haiqi.base.widget.recyclerview.manager.SuperLayoutMananger;
...
//                int y =recyclerView.get();
                View childAt = recyclerView.getChildAt(count - 1);
    ...
    boolean curScroll = true;

    public boolean canScroll(int mHeadHeigh,int mVpHeight,int scrollY){
        if(null == getScrollableView())
            return true;
        ...
        if (getScrollableView() instanceof RecyclerView) {
            ...
            if (layoutManager instanceof LinearLayoutManager) {
                ...
                int count = recyclerView.getAdapter().getItemCount();
//                View childAt = recyclerView.getChildAt(visibleItemPosition);
                // reclerView 的高度
                if(visibleItemPosition == count - 1){
                    View childAt = recyclerView.getChildAt(visibleItemPosition);
                    if (childAt == null) {
                        return true;
                    }
                    int reclerHeight = childAt.getTop() + childAt.getHeight();
                    if(reclerHeight - (mVpHeight - mHeadHeigh) <= scrollY){
                        return false;
                    }
                }
            }
    ...
    public void setScrollEnabled(boolean isScroll){
        if(curScroll == isScroll){
            return;
        }
        this.curScroll = isScroll;
        View scrollableView = getScrollableView();
        if (scrollableView instanceof RecyclerView) {
            RecyclerView mRecyclerView = (RecyclerView)scrollableView;
            if(mRecyclerView.getLayoutManager() instanceof SuperLayoutMananger) {
                SuperLayoutMananger manager = (SuperLayoutMananger) mRecyclerView
                        .getLayoutManager();
                manager.setScrollEnabled(isScroll);
            }
        }
    }
